...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        String line = r.readLine(), replace = line.replaceAll("[0-9]", ".");
        int index = replace.indexOf("."), end = index+1;
        while (index != -1) {
            for (int i = index+1; i < line.length(); i++) {if (replace.charAt(i) == '.') end++;else break;}
            if (line.charAt(index-1) == '+') pr.println(line.substring(0, index-1) + " tighten " + line.substring(index, end));
            else pr.println(line.substring(0, index-1) + " loosen " + line.substring(index, end));
            line = line.substring(end);
            replace = replace.substring(end);
