...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        int a = r.readInt(), b = r.readInt(), trackerX = 0, trackerY = 0, x, y;
        do {
            x = r.readInt(); y = r.readInt();
            if (x == 0 && y == 0) break;
            trackerX = x < 0 ? Math.max(0, trackerX + x) : Math.min(a, trackerX + x);
            trackerY = y < 0 ? Math.max(0, trackerY + y) : Math.min(b, trackerY + y);
            pr.println(trackerX + " " + trackerY);
