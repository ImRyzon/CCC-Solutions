...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = year + 1; i < Integer.MAX_VALUE; i++) {
            ...
            List<Character> alreadyContains = new ArrayList<>();
            for (int j = 0; j < s.length(); j++) {
                if (alreadyContains.contains(s.charAt(j))) {b = false; break;}
                alreadyContains.add(s.charAt(j));
            }
            if (b) {System.out.println(i); break;}
        }
    }
