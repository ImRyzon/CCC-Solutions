...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        int n = r.readInt(), m = r.readInt(), dp[] = new int[n+1];
        Arrays.fill(dp, (int)1e9); dp[0] = 0;
        for (int i = 1; i <= m; i++) {
            int d = r.readInt();
            for (int j = d; j <= n; j++) dp[j] = Math.min(dp[j], dp[j - d] + 1);
        }
        pr.println(dp[n] == (int)1e9 ? "Roberta acknowledges defeat." : "Roberta wins in " + dp[n] + " strokes.");
        pr.close();
    }
