...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        n = r.readInt(); int u = r.readInt(), v = r.readInt(); dp = new int[n+1]; adj = new ArrayList[n+1];
        for (int i = 1; i <= n; i++) adj[i] = new ArrayList<>();
        while (u != 0 || v != 0) {adj[u].add(v); u = r.readInt(); v = r.readInt();}
        pr.println(solve(1)); pr.close();
    }

    static int solve (int u) {
        if (dp[u] != 0) return dp[u];
        if (u == n) return 1;
        if (adj[u].isEmpty()) return 0;
        for (int i : adj[u]) dp[u] += solve(i);
        return dp[u];
    }
